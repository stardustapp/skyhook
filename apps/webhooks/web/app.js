...
Vue.component('create-entry-item', {
  ...
  methods: {
    activate() {
      const type = this.parentName.toLowerCase().replace(/s$/, '');
      editorApp.openEditor({
        type: `create-${type}`,
        label: `create ${type}`,
        icon: 'add',
...
Vue.component('create-script', {
  ...
  methods: {
    async submit() {
      ...
      const data = Skylink.Folder('script', [
        Skylink.String('title', this.title),
        this.upstreamUrl && Skylink.String('upstreamUrl', this.upstreamUrl),
        this.license && Skylink.String('license', this.license),
        Skylink.String('visibility', this.visibility),
      ].filter(x => x));

      const resp = await skylink.invoke(this.tab.path+'/create', data);

      if (resp.Type === 'String') {
        const docId = resp.StringValue;
        const fullPath = this.tab.path + '/' + encodeURIComponent(docId);

        editorApp.openEditor({
          type: "edit-blob",
          icon: "edit",
          label: docId,
          path: fullPath+'/source',
          isNew: true,
          dirty: true,
        });
